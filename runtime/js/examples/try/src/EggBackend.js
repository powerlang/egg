import Egg from "./eggjs/Egg";
import * as logo from "./logo";
import { EggObjectWrapper, EggMethodWrapper } from "./EggObjectWrapper";
...
class EggBackend {
	...
	async initializeRuntime() {
		...
		await this.runtime.loadKernelFile("Kernel.json");
		console.log(this.kernelModule())
		const modules = [
			"Compiler",
			"ImageSegmentBuilder",
			"STON",
		...
		modules.forEach((name) => this.runtime.loadModule(name));
	}
	...
	classTree(rootName, depth, onlyNames = false) {
		let root = rootName
			...
			: this.defaultRootClass();
		if (rootName && !root) this.notFound("class named " + rootName);
	...
	pinObjectSlot(id, path) {
		...
		let id2 = this.newId();
		this.resources.objects[id2] = object;
		let json = object.asWebsideJson();
		json.id = id2;
		return json;
	}
